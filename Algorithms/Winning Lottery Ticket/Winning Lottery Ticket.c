...
{
    unsigned long tktDigitsFreq[0x400] = {0};
    int i, j;
    for(i=0; i<tickets_count; i++)
    {
        unsigned int tktDigits = 0;
        for(j=0; tickets[i][j] && tktDigits < 0x3FF; j++)
            tktDigits |= 1<<(tickets[i][j] - '0');
        tktDigitsFreq[tktDigits]++;
    }
    
    long cnt = tktDigitsFreq[0x3FF];
    if(cnt)
        cnt = (cnt * (cnt - 1))>>1;
    ...
    for(i=0; i<0x3FF; i++)
        ...
        for(j=i+1; j<0x400; j++)
            if((i | j) == 0x3FF)
                cnt += tktDigitsFreq[j] * tktDigitsFreq[i];
