...
char* gridSearch(int G_count, char** G, int P_count, char** P)
{
    ...
    int gridWidth = strlen(G[0]);
    int ptrnWidth = strlen(P[0]);
    
    for(int i = 0; i <= G_count - P_count; i++)
    {
        for(int j = 0; j <= gridWidth - ptrnWidth; j++)
        {
            char isSamePtrn = 1;
            for(int ii = 0; ii < P_count && isSamePtrn; ii++)
            {
                for(int jj = 0; jj < ptrnWidth && isSamePtrn; jj++)
                {
