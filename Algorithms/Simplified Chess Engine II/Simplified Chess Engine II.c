...
Square* piexeNextSqr(Piece (*board)[4], Square sq, int *resultCount)
{
    ...
    *resultCount = 0;
    ...
    {
        case PAWN:
            switch (board[sq.row][sq.col].color)
            {
                case WHITE:
                    if(board[sq.row + 1][sq.col].type == EMPTY)
                    {
                        res[*resultCount].row = sq.row + 1;
                        res[*resultCount].col = sq.col;
                        (*resultCount)++;
                    }
                    
                    if(sq.col < 3 && board[sq.row + 1][sq.col + 1].color == BLACK)
                    {
                        res[*resultCount].row = sq.row + 1;
                        res[*resultCount].col = sq.col + 1;
                        (*resultCount)++;
                    }
                    
                    if(sq.col > 0 && board[sq.row + 1][sq.col - 1].color == BLACK)
                    {
                        res[*resultCount].row = sq.row + 1;
                        res[*resultCount].col = sq.col - 1;
                        (*resultCount)++;
                ...
                case BLACK:
                    ...
                    if(sq.col < 3 && board[sq.row - 1][sq.col + 1].color == WHITE)
                    {
                        res[*resultCount].row = sq.row - 1;
                        res[*resultCount].col = sq.col + 1;
                        (*resultCount)++;
                    ...
                    if(sq.col > 0 && board[sq.row - 1][sq.col - 1].color == WHITE)
                    {
                        res[*resultCount].row = sq.row - 1;
                        res[*resultCount].col = sq.col - 1;
                        (*resultCount)++;
    ...
    for(int i=0; i<4; i++)
    {
        for(int j=0; j<4; j++)
        {
            ...
            for(int k = 0; k < nextPosCnt; k++)
            {
                ...
                //if pawn to be promoted, promote it
                if(board[r][c].type == PAWN && (r == 0 || r == 3))
                {
                    board[r][c].type = ROOK;
