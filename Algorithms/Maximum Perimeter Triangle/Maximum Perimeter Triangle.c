...
int* maximumPerimeterTriangle(int sticks_count, int* sticks, int* result_count)
{
    ...
    *result_count = 1;
    ...
    /* Sort */
    for(int i=0; i<sticks_count-1; i++)
    {
        int minIdx = i;
        for(int j=i; j<sticks_count; j++)
        {
            if(sticks[j] < sticks[minIdx])  minIdx = j;
        }
        int temp = sticks[minIdx];
        sticks[minIdx] = sticks[i];
        sticks[i] = temp;
    }
    
    for(int i=sticks_count-1; i>1; i--)
    {
