...
unsigned long long getWays(int n, int c_count, long* c)
{
    unsigned long long grid[MAX_M][MAX_N + 1] = {0};
    ...
    for(int i=1; i<c_count; i++)
    {
        for(int j=1; j<=n; j++)
        {
            ...
            if(j % c[i] == 0)
                grid[i][j]++;
            for(int k = j - c[i]; k > 0 ; k -= c[i])
                grid[i][j] += grid[i-1][k];
        }
    }
    
    return grid[c_count - 1][n];    //15685693751
}
