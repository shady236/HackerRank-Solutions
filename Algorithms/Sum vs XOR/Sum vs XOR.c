...
long sumXor(long n)
{
    unsigned char zerosCount = 0;
    unsigned char isLeadingOneFound = 0;
    
    for(char i=63; i>=0; i--)
    {
        if(GET_BIT(n, i) == 1)           isLeadingOneFound = 1;
        else if(isLeadingOneFound == 1)  zerosCount++;               
    }
    
    return 1UL<<zerosCount;
}
