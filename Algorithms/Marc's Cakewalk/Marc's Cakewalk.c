...
{
    /* sort */
    for(int i=0; i<calorie_count-1; i++)
    {
        int maxIdx = i;
        for(int j=i+1; j<calorie_count; j++)
        {
            if(calorie[maxIdx] < calorie[j])
                maxIdx = j;
        }
        int temp = calorie[maxIdx];
        calorie[maxIdx] = calorie[i];
        calorie[i] = temp;
    }
