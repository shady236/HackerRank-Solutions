...
int activityNotifications(int expenditure_count, int* expenditure, int d)
{
    ...
    {
        int comulativeFreq = 0;
        if(d % 2 == 0)
        {
            doubleOfMedian = 0;
            for(int j = 0; j < 201; j++)
            {
                comulativeFreq += freq[j];
                
                if(doubleOfMedian == 0 && comulativeFreq >= (d)>>1)
                {
                    doubleOfMedian += j;
                }
                if(comulativeFreq >= (d + 2)>>1)
                {
                    doubleOfMedian += j;
                    break;
                }
        ...
        if(expenditure[i] >= doubleOfMedian)
            count++;
...
int main()
{
    ...
    char** expenditure_temp = split_string(rtrim(readline()));
