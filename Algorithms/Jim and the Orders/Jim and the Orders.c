...
{
    char* isHandled = malloc(orders_rows);
    ...
    *result_count = orders_rows;
    ...
    for(int i=0; i<orders_rows; i++)
    {
        for(int j=0; j<orders_rows; j++)
        {
            if(!isHandled[j])
            {
                if(isHandled[minTimeIdx] || (orders[minTimeIdx][0] + orders[minTimeIdx][1]  >  orders[j][0] + orders[j][1]))
                    minTimeIdx = j;
            }
        }
        res[i] = minTimeIdx+1;
        isHandled[minTimeIdx] = 1;
    }
    return res;
}
