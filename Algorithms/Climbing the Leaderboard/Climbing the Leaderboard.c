...
int* climbingLeaderboard(int ranked_count, int* ranked, int player_count, int* player, int* result_count)
{
    ...
    *result_count = player_count;
    ...
    int* ranks = malloc(ranked_count * sizeof(int));
    ranks[0] = 1;
    for(int i = 1; i < ranked_count; i++)
    {
        ranks[i] = ranks[i-1];
        if(ranked[i] < ranked[i-1])         ranks[i]++;
    }
    
    
    int aheadPlayerIdx = ranked_count - 1;
    for(int i = 0; i < player_count; i++)
    {
        while (aheadPlayerIdx && player[i] > ranked[aheadPlayerIdx])
        {
            aheadPlayerIdx--;
        }
        if(player[i] > ranked[aheadPlayerIdx])
            result[i] = 1;
        else if(ranked[aheadPlayerIdx] == player[i])
            result[i] = ranks[aheadPlayerIdx];
        else 
            result[i] = ranks[aheadPlayerIdx]+1;
    }
