...
unsigned int solve(int arr_count, int* arr) 
{
    unsigned int maxProduct = 0;
    
    int leftIdx  = -1;
    int rightIdx = arr_count;
    ...
    for(int i = 1; i < arr_count - 1; i++)
    {
        ...
        if(rightIdx < arr_count)
        {
            unsigned int prod = (leftIdx + 1) * (rightIdx + 1);
            if(prod > maxProduct)
                maxProduct = prod;
        }
    }
...
int main()
{
    ...
    unsigned int result = solve(arr_count, arr);
