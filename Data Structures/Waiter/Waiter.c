...
int primes[1200] = {0};
void primesInit(int primesCnt)
{
    primes[0] = 2;
    int idx = 1;
    
    for(int p = 3; idx < primesCnt; p += 2)
    {
        if(isPrime(p))
        {
            primes[idx] = p;
...
int* waiter(int number_count, int* number, int q, int* result_count)
{
    ...
    *result_count = 0;
    ...
    char isCopied[50000] = {0};
    char isAllCopied = 0;
    
    for(int i=0; i<q && !isAllCopied; i++)
    {
        isAllCopied = 1;
        if(i % 2 == .0)
        {
            ...
            {
                if(!isCopied[j] && (number[j] % primes[i] == 0))
                {
                    res[*result_count] = number[j];
                    (*result_count)++;
                    isCopied[j] = 1;
                }
                else if(!isCopied[j])
